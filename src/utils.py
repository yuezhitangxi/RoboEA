...
def get_adjr2(ent_size, triples, norm=False):
    print('Generating a dense adjacency matrix...')

    adj = torch.zeros((ent_size, ent_size), dtype=torch.float32)
    neighbours = {}
    ...
    for tri in triples:
        ...
        if tri[0] not in neighbours:
            neighbours[tri[0]] = set()
        if tri[2] not in neighbours:
            neighbours[tri[2]] = set()
        neighbours[tri[0]].add(tri[2])
        neighbours[tri[2]].add(tri[0])
